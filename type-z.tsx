...
const zombieModels = [
  "https://43fzijkfwg2zmvr5.public.blob.vercel-storage.com/zombie1.glb",
  "https://43fzijkfwg2zmvr5.public.blob.vercel-storage.com/zombie2.glb",
  "https://43fzijkfwg2zmvr5.public.blob.vercel-storage.com/zombie3.glb",
  "https://43fzijkfwg2zmvr5.public.blob.vercel-storage.com/zombie4.glb",
  "https://43fzijkfwg2zmvr5.public.blob.vercel-storage.com/zombie5.glb"
]
...
const Zombie = React.memo(({ position, word, currentWord, onDefeat, isActive, modelUrl, onHit, groanUrl, isMuted, listener, gameStarted, gameOver }) => {
  ...
  const hitActionRef = useRef()
  const audioRef = useRef()
  const { scene } = useLoader(GLTFLoader, modelUrl)
  const { animations: walkAnimations } = useLoader(GLTFLoader, modelUrl === zombieModels[3] ? "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/zombie-run-OtQQVYzoknVK3iUyvEgfsDqq9lNlGq.glb" : modelUrl === zombieModels[4] ? "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/monster-walk-Ud0jgDdDuyjqJc7PpJ5w9Z16A6Vgzt.glb" : "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/zombie1-walk2-mxOC2bb3G7PYnBBrvslBU3UJF41Bph.glb")
  ...
  const isZombie4 = modelUrl === zombieModels[3]
  const isZombie5 = modelUrl === zombieModels[4]
  ...
  useEffect(() => {
    ...
    if (walkClip) {
      ...
      if (isZombie4) {
        walkActionRef.current.timeScale = 2.5
      } else if (isZombie5) {
  ...
  }, [clone, walkAnimations, hitAnimations, groanUrl, listener, gameStarted, isMuted, isZombie4, isZombie5])
  ...
  useFrame((state, delta) => {
    if (group.current && isActive) {
      if (!isHitting) {
        ...
        const speed = isZombie4 ? 1.5 : isZombie5 ? 0.7 : 0.5
...
function Game() {
  ...
  const spawnZombiesForWave = useCallback((waveNumber) => {
    ...
    const spawnRadius = 20; // Adjust this value to change how far zombies spawn from the camera

    const getRandomPosition = () => {
      const angle = Math.random() * Math.PI * 2;
      const radius = Math.random() * spawnRadius + 10; // Spawn between 10 and 30 units away
      return [
...
const ZombieModel = ({ onHit, stopGroans, hasHit }) => {
  ...
  useEffect(() => {
    ...
    // Apply grayscale shader to all meshes
    ...
    const animationSequence = async () => {
      if (!hasHit) {
        ...
        await new Promise(resolve => setTimeout(resolve, 2500)) // Reduced run duration
  ...
  useFrame((state, delta) => {
    ...
    if (zombieRef.current && walk) {
      zombieRef.current.position.z += delta * 1.5 // Increased speed
      if (zombieRef.current.position.z >= -1.2) {
        zombieRef.current.position.z = -1.2 // Stop just before the camera
      }
    }
  })
  ...
  return <primitive object={gltf.scene} position={[0, 0, -5]} /> // Start further back
}
